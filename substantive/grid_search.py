	...
	for i, n in enumerate((2, 3, 4, 5, 6)):
		for j, suffix in enumerate(('', '$')):
			for k, binarize in enumerate((True, False)):
				...
				grid1 = GridSearchCV(estimator=LinearSVC(), n_jobs=4, verbose=True,
				...
				X_sg_n_p = Binarizer(copy=False).transform(v_sg.transform(X_sg_n))
				y_sg_n = clf.predict(X_sg_n_p)
				predict_sg[i, j, k] = (y_sg_n == 0).mean()

				grid2 = GridSearchCV(estimator=LinearSVC(), n_jobs=4, verbose=True,
							    	 param_grid={'C': np.logspace(-3, 3, 7)},
				...
				X_pl_n_p = Binarizer(copy=False).transform(v_pl.transform(X_pl_n))
