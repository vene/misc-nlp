...
if __name__ == '__main__':
    ...
    X_sg_n_all = preprocess.load_data('singular_n.txt', labels=False)
    X_pl_n_all = preprocess.load_data('plural_n.txt', labels=False)
    X_sg_n, X_pl_n = [], []
    ...
    for i, n in enumerate((2, 3, 4, 5, 6)):
        for j, suffix in enumerate(('', '$')):
            for k, binarize in enumerate((True, False)):
                ...
                grid1 = GridSearchCV(estimator=LinearSVC(), n_jobs=-1, verbose=True,
                ...
                grid2 = GridSearchCV(estimator=LinearSVC(), n_jobs=-1, verbose=True,
                                     param_grid={'C': np.logspace(-2, 2, 5)},
    ...
    np.save("scores_sg_i", scores_sg)
    np.save("predict_sg_i", predict_sg)
    np.save("best_C_sg_i", best_C_sg)
    np.save("scores_pl_i", scores_pl)
    np.save("predict_pl_i", predict_pl)
    np.save("best_C_pl_i", best_C_pl)
